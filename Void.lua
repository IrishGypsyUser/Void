            ...
            if max then
                if table.find(chosen, option) then
                    ...
                    for _, opt in next, chosen do
                        ...
                        if utility.textlength(table.concat(textchosen, ", ") .. ", ...", newfont, 13).X > (dropdown.AbsoluteSize.X - 18) then
                            cutobject = true
    ...
    set = function(option)
        if max then
            ...
            for _, opt in next, chosen do
                ...
                if utility.textlength(table.concat(textchosen, ", ") .. ", ...", newfont, 13).X > (dropdown.AbsoluteSize.X - 6) then
                    cutobject = true
                    table.remove(textchosen, #textchosen)
                end
            end
...
function library.createkeybind(default, parent, blacklist, flag, callback, offset)
    ...
    local function set(newkey)
        ...
        if newkey ~= nil and not table.find(blacklist, newkey) then
            ...
            local sizeX = utility.textlength(text, newfont, 13).X
            ...
            local text = "[NONE]"
            local sizeX = utility.textlength("[NONE]", newfont, 13).X
    ...
    keybutton.MouseButton1Click:Connect(function()
        if not binding then
            local sizeX = utility.textlength("...", newfont, 13).X
...
function library:Watermark(str)
    local size = utility.textlength(str, newfont, 13).X
    ...
    function windowtypes:Tab(name)
        ...
        function tabtypes:Section(options)
            ...
            function sectiontypes:Separator(name)
                ...
                local sizeX = utility.textlength(name, newfont, 13).X
                ...
                function separatortypes:Set(str)
                    local sizeX = utility.textlength(str, newfont, 13).X
